...
.server-bar { display: flex; align-items: center; gap: 10px; padding: 8px 12px; background: #13131a; border: 1px solid #1e293b; border-radius: 8px; flex-wrap: wrap; flex-shrink: 0; }
.server-label { font-size: 0.775rem; color: #64748b; white-space: nowrap; }
.server-btns { display: flex; gap: 4px; }
.server-btn { padding: 4px 10px; border-radius: 5px; border: 1px solid #1e293b; background: none; color: #64748b; font-size: 0.75rem; cursor: pointer; transition: all 0.15s; }
.server-btn:hover { color: #e2e8f0; border-color: #334155; }
.server-btn.active { color: #3b82f6; border-color: #1d4ed8; background: rgba(59,130,246,0.08); }
.custom-server { display: flex; gap: 4px; }
.server-input { background: #0d0d14; border: 1px solid #1e293b; color: #94a3b8; font-size: 0.75rem; padding: 4px 8px; border-radius: 5px; width: 240px; outline: none; }
.server-input:focus { border-color: #334155; }
.apply-btn { padding: 4px 10px; border-radius: 5px; border: 1px solid #1e293b; background: #1e293b; color: #94a3b8; font-size: 0.75rem; cursor: pointer; }
.apply-btn:hover { color: #e2e8f0; }
.current-url { font-size: 0.7rem; color: #475569; font-family: 'Fira Code', monospace; }

.tester-body { display: flex; gap: 0.75rem; flex: 1; min-height: 0; }
.preset-panel { width: 240px; flex-shrink: 0; background: #0d0d14; border: 1px solid #1e293b; border-radius: 8px; overflow-y: auto; }
.preset-header { padding: 8px 12px; font-size: 0.75rem; color: #475569; text-transform: uppercase; letter-spacing: 0.06em; font-weight: 600; border-bottom: 1px solid #1e293b; }
.preset-item { width: 100%; background: none; border: none; border-bottom: 1px solid #0f172a; padding: 8px 10px; text-align: left; cursor: pointer; display: flex; align-items: flex-start; gap: 6px; transition: background 0.15s; }
.preset-item:hover { background: rgba(255,255,255,0.03); }
.preset-info { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
.preset-path { font-size: 0.75rem; color: #94a3b8; font-family: 'Fira Code', monospace; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.preset-desc { font-size: 0.7rem; color: #475569; }
...
.method-select { width: 90px; flex-shrink: 0; background: #13131a; border: 1px solid #1e293b; border-radius: 6px; font-size: 0.825rem; font-weight: 700; padding: 6px 8px; outline: none; cursor: pointer; font-family: 'JetBrains Mono', monospace; }
.method-get { color: #4ade80; } .method-post { color: #fb923c; } .method-put { color: #60a5fa; } .method-delete { color: #f87171; } .method-patch { color: #c084fc; }
.url-input { flex: 1; background: #13131a; border: 1px solid #1e293b; border-radius: 6px; color: #e2e8f0; font-size: 0.85rem; padding: 6px 12px; outline: none; font-family: 'Fira Code', monospace; }
.url-input:focus { border-color: #334155; }
.send-btn { padding: 6px 20px; background: #1d4ed8; color: #e2e8f0; border: none; border-radius: 6px; font-size: 0.85rem; font-weight: 600; cursor: pointer; transition: background 0.15s; display: flex; align-items: center; gap: 6px; white-space: nowrap; }
.send-btn:hover:not(:disabled) { background: #2563eb; }
.send-btn:disabled { opacity: 0.6; cursor: not-allowed; }
.spinner { width: 12px; height: 12px; border: 2px solid rgba(255,255,255,0.2); border-top-color: white; border-radius: 50%; animation: spin 0.7s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }

.tabs-area { background: #0d0d14; border: 1px solid #1e293b; border-radius: 8px; overflow: hidden; flex-shrink: 0; }
.tabs { display: flex; border-bottom: 1px solid #1e293b; background: #13131a; }
.tab { padding: 6px 14px; background: none; border: none; cursor: pointer; font-size: 0.8rem; color: #64748b; border-bottom: 2px solid transparent; transition: all 0.15s; }
...
.header-input { flex: 1; background: #13131a; border: 1px solid #1e293b; border-radius: 5px; color: #94a3b8; font-size: 0.775rem; padding: 5px 8px; outline: none; font-family: 'Fira Code', monospace; }
...
.clear-btn { background: none; border: none; color: #64748b; font-size: 0.75rem; cursor: pointer; padding: 3px 8px; }
...
.history-url { flex: 1; font-size: 0.75rem; color: #64748b; font-family: 'Fira Code', monospace; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.history-time { font-size: 0.7rem; color: #334155; white-space: nowrap; }
.empty-msg { font-size: 0.8rem; color: #475569; padding: 1rem; text-align: center; }

.preset-method { font-size: 0.65rem; font-weight: 700; padding: 1px 5px; border-radius: 3px; background: rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; flex-shrink: 0; }

.response-panel { flex: 1; min-height: 0; background: #0d0d14; border: 1px solid #1e293b; border-radius: 8px; overflow: hidden; display: flex; flex-direction: column; }
.response-header { padding: 8px 12px; background: #13131a; border-bottom: 1px solid #1e293b; display: flex; align-items: center; gap: 8px; flex-shrink: 0; }
.section-title { font-size: 0.75rem; color: #475569; text-transform: uppercase; font-weight: 600; letter-spacing: 0.05em; }
...
.status-redirect { background: rgba(234,179,8,0.1); color: #fbbf24; }
.duration-badge { font-size: 0.75rem; color: #475569; font-family: 'JetBrains Mono', monospace; }
.res-tabs { display: flex; margin-left: auto; }
.response-body, .response-headers { flex: 1; overflow-y: auto; padding: 1rem; }
.response-pre { font-family: 'Fira Code', monospace; font-size: 0.8rem; color: #94a3b8; white-space: pre-wrap; word-break: break-all; margin: 0; line-height: 1.6; }
.resp-header-row { display: flex; gap: 8px; padding: 4px 0; border-bottom: 1px solid #0f172a; }
.resp-header-key { font-family: 'Fira Code', monospace; font-size: 0.775rem; color: #60a5fa; min-width: 200px; }
.resp-header-val { font-family: 'Fira Code', monospace; font-size: 0.775rem; color: #94a3b8; word-break: break-all; }
.empty-response { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 1rem; padding: 3rem; color: #475569; font-size: 0.85rem; text-align: center; background: #0d0d14; border: 1px solid #1e293b; border-radius: 8px; flex: 1; }
</style>
